...
```
<img width="1014" height="518" alt="image" src="https://github.com/user-attachments/assets/2b56369b-637b-4cc7-9ee9-c7843eb62b29" />
...
```
<img width="1015" height="326" alt="image" src="https://github.com/user-attachments/assets/42d20bf4-9c12-4062-97a8-17f5f8c4a850" />
...
<img width="646" height="133" alt="image" src="https://github.com/user-attachments/assets/3419b106-9b7a-4dc2-8620-e5f94a9e69ce" />

รูปหลังจาก restart postgres
<img width="641" height="213" alt="image" src="https://github.com/user-attachments/assets/bc39f606-9c18-43bd-9d0d-6ab61ff385ae" />
...
<img width="489" height="351" alt="image" src="https://github.com/user-attachments/assets/34d052c1-c5b8-4106-95b5-f08393f9e6f9" />
...
<img width="363" height="114" alt="image" src="https://github.com/user-attachments/assets/11e847a4-f526-49c6-b642-2cf00ebd5a50" />
...
<img width="379" height="108" alt="image" src="https://github.com/user-attachments/assets/d649d534-a487-41d4-bc02-8909725a891e" />
...
### ผลการทดลอง
```
<img width="673" height="182" alt="image" src="https://github.com/user-attachments/assets/13c1e662-d355-43e8-a1a8-00a67b67cc66" />
...
### ผลการทดลอง
```
<img width="645" height="206" alt="image" src="https://github.com/user-attachments/assets/23a36da8-c775-473d-aed9-4d76613ed8d8" />
...
Shared Memory (หน่วยความจำร่วม) คือหน่วยความจำที่ กระบวนการ (process) ต่าง ๆ ของ PostgreSQL ใช้ร่วมกันได้
เช่นการแคชข้อมูล การเก็บข้อมูลของระบบ และการประมวลผลเบื้องต้น
🔹 หน่วยความจำที่เป็น Shared Memory ได้แก่:
shared_buffers — ใช้เก็บข้อมูล (data blocks) ที่ถูกอ่านจากดิสก์ เพื่อให้เรียกใช้งานครั้งถัดไปได้เร็วขึ้น
wal_buffers — ใช้เก็บข้อมูลของ WAL (Write Ahead Log) ก่อนจะเขียนลงดิสก์
work_mem (บางส่วน) — ใช้ในกระบวนการคิวรี เช่นการ sort หรือ hash join แต่โดยทั่วไปเป็น per-session ไม่ได้ shared จริง
maintenance_work_mem — ใช้ในงานบำรุงรักษา เช่น VACUUM, CREATE INDEX, ANALYZE
🧭 หลักในการตั้งค่า:
shared_buffers ≈ 25% ของ RAM ทั้งหมด
wal_buffers ≈ 16MB (หรือมากกว่านั้นในระบบที่เขียนข้อมูลบ่อย)

2. Work memory และ maintenance work memory คืออะไร มีหลักการในการกำหนดค่าอย่างไร
work_mem: หน่วยความจำที่ใช้สำหรับ การคำนวณภายใน query แต่ละตัว เช่น การ SORT, HASH JOIN
เป็น “per operation per connection” หมายความว่า ถ้ามี query 1 ตัวใช้ sort 3 จุด และมี 10 connection
PostgreSQL อาจใช้หน่วยความจำ = 3 × 10 × work_mem
maintenance_work_mem: หน่วยความจำที่ใช้สำหรับ งานบำรุงรักษา เช่น VACUUM, CREATE INDEX, ALTER TABLE ADD FOREIGN KEY, และ ANALYZE
ใช้ครั้งละ 1 กระบวนการ (ไม่คูณตาม connection)
🧮 หลักการตั้งค่า:
work_mem = (RAM × 0.25) ÷ จำนวน connection ที่คาดว่าจะใช้งานพร้อมกัน
...
RAM = 16 GB
Connection = 200
⚙️ ตั้งค่าตัวอื่นก่อน:
shared_buffers ≈ 25% × 16 GB = 4 GB
เหลือ RAM สำหรับ process อื่น ≈ 12 GB
📘 คำนวณ work_mem:
(12 GB ÷ 200 connections) = 0.06 GB = 60 MB
ดังนั้นตั้งได้ประมาณ:
work_mem = 60MB
📗 คำนวณ maintenance_work_mem:
โดยทั่วไป 5–10% ของ RAM
→ 10% × 16GB = 1.6GB
ดังนั้นตั้งได้ประมาณ:
maintenance_work_mem = 1GB

4. ไฟล์ postgresql.conf และ postgresql.auto.conf  มีความสัมพันธ์กันอย่างไร
postgresql.conf → เป็นไฟล์หลักในการตั้งค่าเริ่มต้นของ PostgreSQL
postgresql.auto.conf → เป็นไฟล์ที่ระบบสร้างขึ้นโดยอัตโนมัติ เมื่อใช้คำสั่ง:
ALTER SYSTEM SET parameter = value;
ระบบจะบันทึกค่าใหม่นี้ไว้ใน postgresql.auto.conf โดยไม่ต้องแก้ไฟล์ postgresql.conf ด้วยตนเอง
🧩 ลำดับความสำคัญ:
postgresql.auto.conf จะ override ค่าใน postgresql.conf

5. Buffer hit ratio คืออะไร
Buffer Hit Ratio คือ อัตราส่วนการเข้าถึงข้อมูลจากหน่วยความจำ (cache) โดยไม่ต้องอ่านจากดิสก์จริง
เป็นตัวชี้วัดประสิทธิภาพการใช้ shared_buffers
🧮 สูตรคำนวณ:
SELECT
  (sum(blks_hit)) / (sum(blks_hit) + sum(blks_read)) AS buffer_hit_ratio
FROM pg_stat_database;
✅ ค่าที่ดีควรอยู่ที่ > 0.99 (หรือ 99%) แปลว่าระบบอ่านข้อมูลจาก RAM ได้แทบทั้งหมด
...
Connection = 200
ตัวอย่างการคำนวณ:
Parameter	การคำนวณ	ค่าแนะนำ
shared_buffers	16GB × 0.25	4GB
wal_buffers	~16MB	16MB
work_mem	(12GB ÷ 200)	60MB
maintenance_work_mem	16GB × 0.1	1GB
effective_cache_size	16GB × 0.75	12GB

7. การสแกนของฐานข้อมูล PostgreSQL มีกี่แบบอะไรบ้าง เปรียบเทียบการสแกนแต่ละแบบ
PostgreSQL มีการสแกนข้อมูลหลัก ๆ 3 แบบ:
ประเภทการสแกน	ลักษณะ	ข้อดี	ข้อเสีย
Sequential Scan	อ่านข้อมูลทุกแถวในตาราง	ใช้ได้กับตารางเล็กหรือเมื่อไม่มี index	ช้าเมื่อตารางใหญ่
Index Scan	อ่านเฉพาะแถวที่ตรงเงื่อนไขผ่าน index	เร็วกว่า seq scan มาก	ต้องมี index และอาจช้าหากต้องสุ่มอ่านหลายตำแหน่ง
Bitmap Index Scan	อ่านหลายค่าจาก index แล้วรวมผลก่อนเข้าถึงข้อมูลจริง	ประสิทธิภาพดีเมื่อมีหลายเงื่อนไข OR / AND	ใช้หน่วยความจำมากขึ้น
🔹 PostgreSQL จะเลือกวิธีที่เร็วที่สุดโดยอัตโนมัติจาก Query Planner
